...
Window_Base.prototype.drawCurrentAndMax = function(current, max, x, y,
                                                   width, color1, color2) {
    var labelWidth = this.textWidth('เลือด');
...
Window_Selectable.prototype.processCursorMove = function() {
    if (this.isCursorMovable()) {
        ...
        if (Input.isRepeated('ขวา')) {
            this.cursorRight(Input.isTriggered('ขวา'));
        }
        if (Input.isRepeated('ซ้าย')) {
            this.cursorLeft(Input.isTriggered('ซ้าย'));
        }
...
Window_ItemCategory.prototype.makeCommandList = function() {
    this.addCommand('ไอเท็ม',    'item');
    this.addCommand('อาวุธ',    'weapon');
    this.addCommand('ชุดเกราะ', 'armor');
    this.addCommand('ไอเท็มสำคัญ', 'keyItem');    
};
...
Window_ItemList.prototype.drawItemNumber = function(item, x, y, width) {
    if (this.needsNumber()) {
        this.drawText(':', x, y, width - this.textWidth('00'), 'ขวา');
        this.drawText($gameParty.numItems(item), x, y, width, 'ขวา');
    }
};
...
Window_SkillList.prototype.drawSkillCost = function(skill, x, y, width) {
    if (this._actor.skillTpCost(skill) > 0) {
        ...
        this.drawText(this._actor.skillTpCost(skill), x, y, width, 'ขวา');
    } else if (this._actor.skillMpCost(skill) > 0) {
        this.changeTextColor(this.mpCostColor());
        this.drawText(this._actor.skillMpCost(skill), x, y, width, 'ขวา');
    }
};
...
Window_EquipStatus.prototype.drawCurrentParam = function(x, y, paramId) {
    this.resetTextColor();
    this.drawText(this._actor.param(paramId), x, y, 48, 'ขวา');
};
...
Window_EquipStatus.prototype.drawNewParam = function(x, y, paramId) {
    ...
    this.drawText(newValue, x, y, 48, 'ขวา');
};
...
Window_Status.prototype.drawParameters = function(x, y) {
    ...
    for (var i = 0; i < 6; i++) {
        ...
        this.resetTextColor();
        this.drawText(this._actor.param(paramId), x + 160, y2, 60, 'ขวา');
    }
};
...
Window_Status.prototype.drawExpInfo = function(x, y) {
    ...
    this.resetTextColor();
    this.drawText(value1, x, y + lineHeight * 1, 270, 'ขวา');
    this.drawText(value2, x, y + lineHeight * 3, 270, 'ขวา');
};
...
Window_Options.prototype.drawItem = function(index) {
    ...
    this.resetTextColor();
    this.changePaintOpacity(this.isCommandEnabled(index));
    this.drawText(this.commandName(index), rect.x, rect.y, titleWidth, 'ซ้าย');
    this.drawText(this.statusText(index), titleWidth, rect.y, statusWidth, 'ขวา');
};
...
Window_SavefileList.prototype.drawPlaytime = function(info, x, y, width) {
    if (info.playtime) {
        this.drawText(info.playtime, x, y, width, 'ขวา');
    }
};
...
Window_ShopNumber.prototype.drawNumber = function() {
    ...
    this.resetTextColor();
    this.drawText(this._number, x, y, width, 'ขวา');
};
...
Window_ShopStatus.prototype.drawPossession = function(x, y) {
    ...
    this.resetTextColor();
    this.drawText($gameParty.numItems(this._item), x, y, width, 'ขวา');
};
...
Window_ShopStatus.prototype.drawActorParamChange = function(x, y, actor, item1) {
    ...
    this.drawText((change > 0 ? '+' : '') + change, x, y, width, 'ขวา');
};
...
Window_TitleCommand.prototype.makeCommandList = function() {
    this.addCommand('เริ่มต้นการไปโรงเรียนใหม่', 'newGame');
    this.addCommand('เล่นต่อจากเดิม', 'continue', this.isContinueEnabled());
    this.addCommand('ตั้งค่า', 'options');
};
...
Window_DebugEdit.prototype.drawItem = function(index) {
    ...
    this.drawText(name, rect.x, rect.y, rect.width);
    this.drawText(status, rect.x + rect.width, rect.y, statusWidth, 'ขวา');
};
